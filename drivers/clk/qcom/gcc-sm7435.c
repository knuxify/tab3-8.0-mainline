...
#include <dt-bindings/clock/qcom,gcc-sm7435.h>
...
// TODO: Pretty much all of these have enable_safe_config enabled, which does
//       not seem to have a mainline counterpart.
//       Might need to change ops to &clk_rcg2_shared_ops?
//       https://lore.kernel.org/lkml/bb3cdcfa-943e-074d-04f6-bfd8b2cd378d@linaro.org/
...
static struct clk_regmap_div gcc_qupv3_wrap0_s5_div_clk_src = {
	.reg = 0x2774c,
	.shift = 0,
	.width = 4,
	.clkr.hw.init = &(struct clk_init_data) {
		.name = "gcc_qupv3_wrap0_s5_div_clk_src",
		.parent_hws = (const struct clk_hw*[]){
...
static struct clk_regmap_div gcc_qupv3_wrap1_s5_div_clk_src = {
	.reg = 0x28744,
	.shift = 0,
	.width = 4,
	.clkr.hw.init = &(struct clk_init_data) {
		.name = "gcc_qupv3_wrap1_s5_div_clk_src",
		.parent_hws = (const struct clk_hw*[]){
...
static struct clk_branch gcc_camera_sleep_clk = {
	.halt_reg = 0x36018,
...
static struct clk_branch gcc_qupv3_wrap0_s5_clk = {
	...
	.clkr = {
		...
		.hw.init = &(struct clk_init_data){
			...
			.parent_hws = (const struct clk_hw*[]){
				&gcc_qupv3_wrap0_s5_div_clk_src.clkr.hw,
			},
			.num_parents = 1,
			.flags = CLK_SET_RATE_PARENT,
			.ops = &clk_branch2_ops,
...
static struct clk_branch gcc_qupv3_wrap1_s5_clk = {
	...
	.clkr = {
		...
		.hw.init = &(struct clk_init_data){
			...
			.parent_hws = (const struct clk_hw*[]){
				&gcc_qupv3_wrap1_s5_div_clk_src.clkr.hw,
			},
			.num_parents = 1,
			.flags = CLK_SET_RATE_PARENT,
			.ops = &clk_branch2_ops,
...
static struct clk_regmap *gcc_sm7435_clocks[] = {
	...
	[GCC_CAMERA_SLEEP_CLK] = &gcc_camera_sleep_clk.clkr,
	...
	[GCC_QUPV3_WRAP0_S5_DIV_CLK_SRC] = &gcc_qupv3_wrap0_s5_div_clk_src.clkr,
	...
	[GCC_QUPV3_WRAP1_S5_DIV_CLK_SRC] = &gcc_qupv3_wrap1_s5_div_clk_src.clkr,
...
static const struct qcom_reset_map gcc_sm7435_resets[] = {
	[GCC_CAMERA_BCR] = { 0x36000 },
	[GCC_DISPLAY_BCR] = { 0x37000 },
	[GCC_EMMC_BCR] = { 0xb3000 },
	[GCC_GPU_BCR] = { 0x81000 },
	...
	[GCC_VIDEO_AXI0_CLK_ARES] = { 0x4200c, 2 },
...
static const struct regmap_config gcc_sm7435_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_sm7435_desc = {
	.config = &gcc_sm7435_regmap_config,
	.clks = gcc_sm7435_clocks,
	.num_clks = ARRAY_SIZE(gcc_sm7435_clocks),
	.resets = gcc_sm7435_resets,
	.num_resets = ARRAY_SIZE(gcc_sm7435_resets),
};

static const struct of_device_id gcc_sm7435_match_table[] = {
	{ .compatible = "qcom,gcc-sm7435" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_sm7435_match_table);

static int gcc_sm7435_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_sm7435_desc);
	if (IS_ERR(regmap))
	...
	// TODO, not sure what bits these are?
	...
	return qcom_cc_really_probe(&pdev->dev, &gcc_sm7435_desc, regmap);
}

static struct platform_driver gcc_sm7435_driver = {
	.probe = gcc_sm7435_probe,
	.driver = {
		.name = "gcc-sm7435",
		.of_match_table = gcc_sm7435_match_table,
	},
};

static int __init gcc_sm7435_init(void)
{
	return platform_driver_register(&gcc_sm7435_driver);
}
subsys_initcall(gcc_sm7435_init);

static void __exit gcc_sm7435_exit(void)
{
	platform_driver_unregister(&gcc_sm7435_driver);
}
module_exit(gcc_sm7435_exit);

MODULE_DESCRIPTION("QTI GCC SM7435 Driver");
